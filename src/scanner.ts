...
async function walkDirectory(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      await walkDirectory(fullPath, rootPath, gitignore, excludeFilter, results);
...
export async function scan(
  rootPath: string,
  options: ScanOptions = { include: [], exclude: [] },
): Promise<string[]> {
  ...
  const excludeFilter = createExcludeFilter(options.exclude);

  // Create include filter
  const includeFilter = options.include.length > 0
    ? createExcludeFilter(options.include)
    : null;

  // Walk the directory tree
  const allFiles: string[] = [];
  await walkDirectory(absRoot, absRoot, gitignore, excludeFilter, allFiles);

  // Apply include filter: keep only files that match include patterns
  let filtered = allFiles;
  if (includeFilter) {
    filtered = allFiles.filter((f) => matchesInclude(f, includeFilter));
  }
